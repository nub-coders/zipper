...
async def without_pass(event):
    pass_protect = None
    await event.edit("starting without password")
...
# Print the result
    await event.respond("**Storage is limited to 200 MB for non verified users**\nyou need to verify first in order to incrrase storage capacity\n\nThis is only file to zip bot which gives 4.5 GB storage support to the user \n\nYou can also use /premium to get many benifits including no ads",buttons=[Button.url("Click to verify",links[output]),Button.url("how to verify","https://t.me/nub_coder_updates/211")])
    if not premium_queue.empty():
